...
const lastUpdateLabel = computed(() =>
  lastUpdate.value.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })
);

// classes fixas (exemplo de card “premium”)
const TIP = {
  card:
...
<template>
  <div class="space-y-4">
    ...
    <div v-if="data" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
      <MetricCard label="Revenue" :value="`€ ${data.kpis.revenue.toFixed(0)}`" trend="LIVE" highlight>
        <template #icon>
          <AppIcon name="Banknote" className="text-green-500" />
        </template>
      </MetricCard>

      <MetricCard label="Sessions" :value="data.kpis.sessions.toLocaleString()" trend="TODAY">
        <template #icon>
          <AppIcon name="Users" className="text-blue-500" />
        </template>
      </MetricCard>

      <MetricCard label="Conversion" :value="`${data.kpis.conversion.toFixed(2)}%`" trend="WEEK">
        <template #icon>
          <AppIcon name="TrendingUp" className="text-orange-500" />
        </template>
      </MetricCard>

      <MetricCard label="Errors" :value="`${data.kpis.errors}`" trend="24H">
        <template #icon>
          <AppIcon name="TriangleAlert" className="text-red-500" />
        </template>
      </MetricCard>

      <MetricCard label="Latency" :value="`${data.kpis.latency.toFixed(0)} ms`" trend="LIVE">
        <template #icon>
          <AppIcon name="Gauge" className="text-purple-500" />
        </template>
      </MetricCard>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 xl:gap-6 2xl:gap-7 items-stretch">
      ...
      <div class="md:col-span-8 bg-theme-surface border border-theme-border/30 p-5 xl:p-7 2xl:p-8 rounded-3xl shadow-md h-full flex flex-col">
        <div class="flex items-start justify-between mb-4">
          <div>
            <h3 class="font-bold text-theme-text">Performance history</h3>
            <p class="text-xs text-theme-muted">Example chart with two series (24 points)</p>
          </div>

          <div class="flex items-center gap-4 text-[10px] font-bold uppercase tracking-wider text-theme-muted">
            <div class="flex items-center gap-1.5">
              <span class="w-2.5 h-2.5 rounded-full bg-blue-500"></span> SERIES A
            </div>
            <div class="flex items-center gap-1.5">
              <span class="w-2.5 h-2.5 rounded-full bg-orange-400"></span> SERIES B
            </div>
          </div>
        </div>
      ...
      <div class="md:col-span-4 flex flex-col gap-4 xl:gap-6 h-full">
        ...
        <div class="bg-theme-surface border border-theme-border/30 p-6 xl:p-7 rounded-3xl shadow-md">
          <div class="flex flex-col items-center text-center">
            ...
            <h3 class="text-lg font-bold text-theme-text mb-1">System health</h3>
            ...
            <div class="grid grid-cols-2 gap-3 w-full">
              ...
              <div class="p-3 rounded-2xl text-center bg-black/5 dark:bg-white/10">
                <p class="text-theme-muted text-[10px] font-bold uppercase mb-1">INCIDENTS</p>
          ...
          <div class="relative z-10 flex items-start gap-3">
            <div class="shrink-0 mt-0.5">
              <div class="w-9 h-9 rounded-xl flex items-center justify-center" :class="TIP.iconWrap">
                <AppIcon name="Lightbulb" :size="18" :className="TIP.icon" />
              </div>
            </div>

            <div class="min-w-0">
              <h4 :class="TIP.title">Boilerplate tip</h4>
              <p :class="TIP.text">
                This is a “premium” card example. Use it for notices, tips or onboarding.
              </p>
            </div>
          </div>

          <AppIcon name="Lightbulb" :size="96" :className="TIP.bgIcon" />
        </div>
      </div>
    </div>
  </div>
